...
void DxlROSBaseControl::ps3Callback(const std_msgs::String::ConstPtr& msg)
{
  ...
  double gas = atof( strs[0].c_str() );
  double power = atof(strs[5].c_str());
  drive(gas,power,angular);
}
...
void DxlROSBaseControl::publishOdometry()
{
	...
	double left = (motor1_->presentLinearPos() - prevLeft_)*1.01;
	double right = (motor2_->presentLinearPos() - prevRight_)*1.01;
