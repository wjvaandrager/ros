...
*
*/
const char *ws_protocol_ver = "1.0.3";
...
int connectToServer(CPhidgetRemoteHandle remoteInfo, char *errdesc, int errlen, void *list_element, ListElementType type)
{
	...
	switch(result)
	{
		...
		case EPHIDGET_NOTFOUND: //Not Found
			...
			if (!stream_server_connect(addr_lookup_str, newServerInfo->server->port, &newServerInfo->server->socket, errdesc, errlen))
			{
#else
			if (!stream_server_connect(newServerInfo->server->address, newServerInfo->server->port, &newServerInfo->server->socket, errdesc, errlen))
			{
#endif
			...
			//add it to the list - not we are allowed to have connecting servers in the list!
...
error_event:
	...
	inErrorEvent = PTRUE;
	throw_error_event(err_device, errdesc, result);
	inErrorEvent = PFALSE;
	return result;
}
...
int closeServer(CServerInfoHandle server)
{
	...
	if(!server->phidgets && !server->dictionaries && !server->managers && pdcs)
	{
