...
CPHIDGETINIT(IR)
	...
	//reset data pointers
	phid->dataReadPtr = 0;
	phid->dataBuffer[0] = PUNK_INT; //this lets it recognize the first code after init
	phid->dataWritePtr = 1;
	phid->userReadPtr = 0;
	phid->learnReadPtr = 0;
...
CPHIDGETDATA(IR)
	...
	#endif
		if(duration > IR_MAX_GAP_LENGTH/1000000)
		{
...
analyze_step_two:
	...
	//chould be one more high then low
...
PHIDGET21_API int CCONV CPhidgetIR_TransmitRepeat(CPhidgetIRHandle phid)
{
	...
	//assume that lastt code is valid
	if(dataSize > 0)
	{
		if(CPhidget_statusFlagIsSet(phid->phid.status, PHIDGET_REMOTE_FLAG))
			;//ADDNETWORKKEYINDEXED(Acceleration, "%lE", motorAcceleration); //TODO
