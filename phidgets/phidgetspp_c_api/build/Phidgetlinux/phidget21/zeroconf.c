...
pthread_t dns_thread_ws = NULL, dns_thread_phid = NULL, dns_thread_sbc = NULL;
...
int InitializeZeroconf()
{
	...
	if(!Dns_sdBrowsing)
	{
		...
		if (error == kDNSServiceErr_NoError)
        {
			pthread_create(&dns_thread_sbc, NULL, (void *(*)(void *))dns_callback_thread,zeroconf_browse_sbc_ref);
        }
		else
		{
...
int UninitializeZeroconf()
{
	...
	if(Dns_sdBrowsing)
	{
		...
		if(dns_thread_ws)
		{
			pthread_join(dns_thread_ws, &status);
			dns_thread_ws = NULL;
		}
		if(dns_thread_sbc)
		{
			pthread_join(dns_thread_sbc, &status);
			dns_thread_sbc = NULL;
		}
		if(dns_thread_phid)
		{
			pthread_join(dns_thread_phid, &status);
			dns_thread_phid = NULL;
		}
