...
CPHIDGETINIT(TemperatureSensor)
		...
		case PHIDID_TEMPERATURESENSOR:
			if(phid->phid.deviceVersion < 200) 
			{
				for (i = 0; i<phid->phid.attr.temperaturesensor.numTempInputs; i++)
				{
					...
					phid->temperatureMax[i] = 1250;
					phid->temperatureMin[i] = -200;
				}
	...
	for (i = 0; i<phid->phid.attr.temperaturesensor.numTempInputs; i++)
	{
		if((lookup_voltage(phid->temperatureMax[i], phid->ThermocoupleType[i]) - lookup_voltage(phid->ambientTemperatureMax, phid->ThermocoupleType[i])) > phid->potentialMax)
			phid->temperatureMax[i] = round_double((lookup_temperature(phid->potentialMax, phid->ThermocoupleType[i]) + phid->ambientTemperatureMax), 4);
		if((lookup_voltage(phid->temperatureMin[i], phid->ThermocoupleType[i]) - lookup_voltage(phid->ambientTemperatureMin, phid->ThermocoupleType[i])) < phid->potentialMin)
			phid->temperatureMin[i] = round_double((lookup_temperature(phid->potentialMin, phid->ThermocoupleType[i]) + phid->ambientTemperatureMin), 4);
	}
...
CPHIDGETDATA(TemperatureSensor)
	int i = 0;
	double Vad = 0, Vthermocouple = 0, VtempIC = 0;
	...
	switch(phid->phid.deviceIDSpec)
	{
		case PHIDID_TEMPERATURESENSOR:
			...
			/* newer versions... calculate the temp here from Vad */
				...
				Temperature[0] = round_double(lookup_temperature(Vthermocouple, phid->ThermocoupleType[0]), 3);
			}
			...
			else if(phid->phid.deviceVersion >= 300 && phid->phid.deviceVersion < 400)
			{
				...
				//This is the potential at the thermocouple tip - with the junction potential eliminated
				Vthermocouple = Potential[0] + lookup_voltage(AmbientTemperature, phid->ThermocoupleType[0]);
				Temperature[0] = round_double(lookup_temperature(Vthermocouple, phid->ThermocoupleType[0]), 4);
			}
			else if(phid->phid.deviceVersion >= 400 && phid->phid.deviceVersion < 500) 
			{
				//Temp IC - absolute possible range: -61.1 - 161.16
		...
		case PHIDID_TEMPERATURESENSOR_4:
			if(phid->phid.deviceVersion >= 100 && phid->phid.deviceVersion < 200) 
			{
				...
				for (i = 0; i<phid->phid.attr.temperaturesensor.numTempInputs; i++)
				{
					...
					Temperature[i] = round_double(lookup_temperature(Vthermocouple, phid->ThermocoupleType[i]), 4);
	...
	for (i = 0; i<phid->phid.attr.temperaturesensor.numTempInputs; i++)
	{
		...
		if(Temperature[i] > phid->temperatureMax[i])
		{
			phid->Temperature[i] = PUNK_DBL;
		...
		else if(phid->AmbientTemperature == PUNK_DBL)
		{
			phid->Temperature[i] = PUNK_DBL;
			if (phid->phid.fptrError && phid->TemperatureError[i] == PFALSE)
			{
...
CSETINDEX(TemperatureSensor,ThermocoupleType, CPhidgetTemperatureSensor_ThermocoupleType)
	...
	{	
		case PHIDGET_TEMPERATURE_SENSOR_K_TYPE:
			phid->temperatureMax[Index] = 1250;
			phid->temperatureMin[Index] = -200;
			break;
		case PHIDGET_TEMPERATURE_SENSOR_J_TYPE:
			phid->temperatureMax[Index] = 750;
			phid->temperatureMin[Index] = -40;
			break;
		case PHIDGET_TEMPERATURE_SENSOR_E_TYPE:
			phid->temperatureMax[Index] = 900;
			phid->temperatureMin[Index] = -200;
			break;
		case PHIDGET_TEMPERATURE_SENSOR_T_TYPE:
			phid->temperatureMax[Index] = 350;
			phid->temperatureMin[Index] = -200;
			break;
		default:
			return 0;
	...
	if((lookup_voltage(phid->temperatureMax[Index], newVal) - lookup_voltage(phid->ambientTemperatureMax, newVal)) > phid->potentialMax)
		phid->temperatureMax[Index] = round_double((lookup_temperature(phid->potentialMax, newVal) + phid->ambientTemperatureMax), 4);
	if((lookup_voltage(phid->temperatureMin[Index], newVal) - lookup_voltage(phid->ambientTemperatureMin, newVal)) < phid->potentialMin)
		phid->temperatureMin[Index] = round_double((lookup_temperature(phid->potentialMin, newVal) + phid->ambientTemperatureMin), 4);
