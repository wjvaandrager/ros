...
void DxlROSBaseNav::publishOdometry()
{
	...
	double left = (motor1_->presentPos() - prevLeft_)/(2*M_PI);
	double right = (motor2_->presentPos() - prevRight_)/(2*M_PI);
...
// 	double dt = (current_time_ - last_time_).toSec();
	...
	odom_pub_.publish(odom);
        prevLeft_ = motor1_->presentPos();
	prevRight_ = motor2_->presentPos();
...
void DxlROSBaseNav::velCallback(const geometry_msgs::Twist::ConstPtr& msg)
{
	...
	double vel_angular = msg->angular.z * ((2*wheel_base_)/wheel_diameter_);
        if (vel_linear > 3){
	  vel_linear = 3;
        }
